...
Setup ~org-ref~ to use a single org file per bibliographic entry.
...
Define the ~org-ref~ template to include ~ROAM_KEY~.
...
While I still use Zotero to grab citation data, I export that content to a
single biblatex file.
